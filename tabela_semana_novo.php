<?php
    ...
    // Data gerada pelo usuario: Essa data tenha que gerar automaticamento e a opição do usuario escolher a semana
    $data_digitada = ' 2020-10-26';
    ...
    // Gerando os dias da semana mostrada na tabela:
    $semana = gerarSemana(new DateTime($data_digitada)); 
    // Gerando todos os horarios da tabela:
    $lista_horarios = gerarHorarios($semana[0], 8, 21);
    
    // Resultado: lista de objetos Table de um unico
    $array_teste_tabela = gerarObjTabela($lista_horarios); 
    
    $array_comparacao = compararObjTabela($_SESSION['tbLista'], $array_teste_tabela);

    // GERANDO TODAS AS LINHAS DA TABELA: AUTOMATIZA DEPOIS!
